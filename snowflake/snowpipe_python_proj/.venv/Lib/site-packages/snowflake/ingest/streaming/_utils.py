...
def convert_bytes_in_row(
    row: Dict[str, Any], binary_input_format: "PyBinaryInputFormat"
) -> Dict[str, Any]:
    """Convert bytes/bytearray values in a row dict according to binary_input_format.

    For HEX, byte values become hex-encoded strings.
    For UTF-8, byte values are decoded as UTF-8 strings.
    ...
    for key, value in row.items():
        if isinstance(value, (bytes, bytearray)):
            if binary_input_format == PyBinaryInputFormat.HEX:
                result[key] = value.hex()
            elif binary_input_format == PyBinaryInputFormat.UTF8:
