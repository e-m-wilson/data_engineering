...
class StreamingIngestChannel:
    ...
    def __init__(
        ...
        *,
        binary_input_format: PyBinaryInputFormat = PyBinaryInputFormat.BASE64,
        ...
        Args:
            ...
            pending_futures: A dictionary that maps pending Future IDs to their respective Future.
            binary_input_format: The encoding format for binary column values.
                One of PyBinaryInputFormat.BASE64, HEX, or UTF8.
    ...
    def append_rows(
        ...
        self._channel.append_rows(
            json_bytes, len(rows), False, start_offset_token, end_offset_token
        )

    def _append_row_with_wait(
        self,
        row: Dict[str, Any],
        offset_token: Optional[str] = None,
    ) -> Future:
        """Append a single row into the channel and return a concurrent.futures.Future that completes when the row has been acknowledged by the Snowflake server.
        ...
        Returns:
            Future: Completes on success if successfully acknowledged by Snowflake

        """
        return self._append_rows_with_wait([row], offset_token, offset_token)

    @_rethrow_ffi_errors
    def _append_rows_with_wait(
        self,
        rows: List[Dict[str, Any]],
        start_offset_token: Optional[str] = None,
        end_offset_token: Optional[str] = None,
    ) -> Future:
        """Append multiple rows into the channel and return a concurrent.futures.Future that completes when the row has been acknowledged by the Snowflake server.
        ...
        future_id = self._channel.append_rows(
            json_bytes, len(rows), True, start_offset_token, end_offset_token
        )
        ...
        if previous is not None:
            ...
            self._invalidate_channel(error_msg)
            raise error
    ...
    def close(
        ...
        # clean up futures only on close() success
        for future in self._pending_futures.values():
            if not future.done():
                future.set_exception(
                    StreamingIngestError(
                        StreamingIngestErrorCode.CLOSED_CHANNEL_ERROR,
                        "Channel closed with pending futures",
                        HTTPStatus.CONFLICT.value,
    ...
    def _invalidate_channel(self, error_msg: str) -> None:
