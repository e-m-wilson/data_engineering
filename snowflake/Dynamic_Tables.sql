...
CREATE OR REPLACE DYNAMIC TABLE customer_spend_summary
    TARGET_LAG = '1 minute' -- how long until a refresh happens
    WAREHOUSE = compute_wh -- refreshing requires compute 
    REFRESH_MODE = AUTO -- INCREMENTAL, FULL, AUTO 
    -- INCREMENTAL = only updates table with what has changes; FULL = re-writes entire table, AUTO = snowflake analyzes the query and chooses for us
    INITIALIZE = ON_CREATE -- table populated immediately on init
AS
    SELECT
        customer_id,
