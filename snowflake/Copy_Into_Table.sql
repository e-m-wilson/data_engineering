...
----------------------------------------------*/
-- ASSUMING WE STILL HAVE THE STAGES FROM PREVIOUS HANDS ON
--Set context
USE ROLE SYSADMIN;
...
-- The first is RETURN_n_ROWS = This allows us to test a subset of the rows in an input file.
-- RETURN_ERRORS - This returns all errors on all rows of all input files attempting to be loaded.
-- RETURN_ALL_ERRORS = This is the same as the previous value, but includes those records that have failed as part of a partial
-- load to validate past executions of the copy into table command.
COPY INTO FILMS FROM @FILMS_STAGE/films.csv
...
SELECT * FROM TABLE(VALIDATE(FILMS, job_id=>'<failed_job_id_query_id_in_vscode>'));